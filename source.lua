...
-- ========================================================
-- Dynamic Asset Downloader & Resolver (Xeno / GitHub Fix)
-- ========================================================
local function resolveImage(assetId, localPath)
    ...
    if type(assetId) == "string" then
        ...
        if string.find(assetId, "%.png") or string.find(assetId, "%.jpg") then
            local fileName = assetId:match("([^/]+)$") or assetId
            local rawUrl = "https://raw.githubusercontent.com/nadmksa123456-lgtm/TRust_Menu/main/assets/" .. fileName
            return fetchAndCache(rawUrl, fileName)
        end
    end
...
function Library:CreateWindow(options)
    ...
    local requestedSize = options.Size or Vector2.new(1000, 750)
    local activeCamera = Workspace.CurrentCamera or Camera
    ...
    local sidebarWidth = options.SidebarWidth or 84
    local topbarHeight = options.TopbarHeight or 72
    ...
    -- Default Logo Setup (Eagle 0.png)
    local defaultLogoUrl = "https://raw.githubusercontent.com/nadmksa123456-lgtm/TRust_Menu/main/assets/0.png"
    local logoInput = options.Logo or options.LogoUrl
    local logoFileInput = options.LogoFile
    if (not logoInput or logoInput == "") and (not logoFileInput or logoFileInput == "") then
        logoInput = defaultLogoUrl
        logoFileInput = "0.png"
    end

    local logoImage = resolveImage(logoInput, logoFileInput)
    ...
    function window:Category(categoryOptions)
        ...
        local iconAsset = resolveImage(categoryOptions.Icon or categoryOptions.IconUrl, categoryOptions.IconFile)
